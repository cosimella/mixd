        ...
        foreach ($_FILES['new_images']['tmp_name'] as $i => $tmp) {
            if ($_FILES['new_images']['error'][$i] === 0) {
                ...
                if (move_uploaded_file($tmp, $uploadDir . $file)) {
                    ...
                    $conn->query("INSERT INTO recipe_images (recipe_id, image_path) VALUES ($idRecipe, '$fullPath')");
                }
            }
        }
    }
            ...
            <div class="row g-4">
                ...
                <div class="col-md-8">
                    <div class="card border-0 shadow-sm p-4 rounded-4 mb-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="recipe_name" class="form-control form-control-custom" value="<?= htmlspecialchars($recipe['recipe_name']) ?>" required>
                        </div>
                        <div class="mb-0">
                            <label class="small fw-bold text-muted">BESCHREIBUNG</label>
                            <textarea name="beschreibung" class="form-control form-control-custom" rows="3"><?= htmlspecialchars($recipe['beschreibung']) ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="card border-0 shadow-sm p-4 rounded-4 mb-4">
                        ...
                        <div class="row g-2 mb-2 align-items-center">
                            <div class="col-2">
                                <input type="number" step="0.1" name="amount[]" class="form-control form-control-custom text-center" value="<?= $ing ? (float)$ing['amount'] : '' ?>" placeholder="Anz.">
                            </div>
                            <div class="col-3">
                            ...
                            <div class="col-7">
                                <input type="text" name="ingredient[]" class="form-control form-control-custom" value="<?= $ing ? htmlspecialchars($ing['ingredient_name']) : '' ?>" placeholder="Zutat <?= $i+1 ?>">
                            </div>
                        </div>
                        <?php endfor; ?>
                    </div>
                ...
                <div class="col-md-4">
                    <div class="card border-0 shadow-sm p-4 rounded-4 mb-4">
                            ...
                            <?php foreach ($images as $img): ?>
                                <div class="text-center" style="width: 80px;">
                                    ...
                                    <div class="form-check">
                                        <input class="form-check-input mx-auto" type="checkbox" name="delete_images[]" value="<?= $img['image_id'] ?>">
                        ...
                        <input type="file" name="new_images[]" class="form-control form-control-sm form-control-custom" multiple>
                    </div>

                    <button type="submit" name="save_recipe" class="btn btn-primary w-100 rounded-pill py-3 fw-bold shadow-sm">
                        ÄNDERUNGEN SPEICHERN
                    </button>
                    
                    <div class="text-center mt-3">
                        <a href="recipe.php?id=<?= $idRecipe ?>" class="text-muted small text-decoration-none">Abbrechen & Zurück</a>
                    </div>
                </div>
            </div>
        </form>
